 ...
 */
export const createTable = pgTableCreator((name) => `pay-as-you-go-v2_${name}`);
...
export const usersRelations = relations(users, ({ many }) => ({
  ...
  storagePurchases: many(storageUserPurchase), // Fixed relation name to match the table
}));
...
export const storageFiles = createTable("storage_files", {
  id: varchar("id", { length: 255 })
    .notNull()
    .primaryKey()
    .$defaultFn(() => crypto.randomUUID()),
...
export const fileSharing = createTable("file_sharing", {
  ...
  sharedWithId: varchar("shared_with_id", { length: 255 })
    .notNull()
    .references(() => users.id),
  createdAt: timestamp("created_at", { withTimezone: true })
    .default(sql`CURRENT_TIMESTAMP`)
...
export const storageUserPurchase = createTable("storage_user_purchase", {
  id: varchar("id", { length: 255 })
    .notNull()
    .primaryKey()
    .$defaultFn(() => crypto.randomUUID()),
...
export const storageUserPurchaseRelations = relations(
  storageUserPurchase,
  ({ one }) => ({
    user: one(users, {
      fields: [storageUserPurchase.userId],
      references: [users.id],
    }),
  }),
);
